...
def log_end(task_id: str, total_reward: float, rewards: list[float],
    ...
    # Clamp score to strictly (0.0, 1.0) exclusive as required by evaluator
...
def run_task_multi_agent(task_id: str) -> None:
    """Run the multi-agent orchestrator for a task, emitting standard [START]/[STEP]/[END] logs."""
    ...
    orchestrator.reset(task_id=task_id)
    ...
    while not done:
        ...
        # Get classification history for context memory
        ...
        # Emit standard log with coordinator info in breakdown
        enriched_breakdown = dict(reward.breakdown)
        enriched_breakdown["coordinator_score"] = round(trace_step.coordinator_score, 3)

        log_step(task_id, step_num, action.action_type, action.value,
                 reward.step_reward, reward.episode_reward, enriched_breakdown, done)
        step_num += 1
    ...
    if final_info.get("metrics"):
        final_info["metrics"].update(ep_metrics)
...
def main() -> None:
    ...
    parser.add_argument("--multi-agent", action="store_true",
                        help="Use multi-agent orchestrator (CoordinatorAgent + self-improving memory)")
    args = parser.parse_args()

    # Auto-detect dry-run if no API key is available
    api_key = os.environ.get("HF_TOKEN") or os.environ.get("OPENAI_API_KEY")
    use_dry_run = args.dry_run or not api_key

    if args.multi_agent:
        for task_id in TASKS:
            run_task_multi_agent(task_id)
        return
